...
type CreateDatasCommand struct {
	BaseCommand
...
func (cmd *CreateDatasCommand) Run(pctx context.Context) error { // nolint:dupl
...
func (cmd *CreateDatasCommand) parseFlags() error {
...
func (cmd *CreateDatasCommand) createOperation() (base.Operation, error) { // nolint:dupl
	e := util.StringError("failed to create create-datas operation")

	item := storage.NewCreateDatasItem(cmd.contract, cmd.Key, cmd.Value, cmd.Currency.CID)

	fact := storage.NewCreateDatasFact([]byte(cmd.Token), cmd.sender, []storage.CreateDatasItem{item})

	op, err := storage.NewCreateDatas(fact)
	if err != nil {
